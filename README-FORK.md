...
- **Gentle AI (SDD agents + skills):** `gentle-ai/README.md` — orchestrator `sdd-orchestrator` and `sdd-*` sub-agents are defined in `.opencode/opencode.jsonc`; skills path `gentle-ai/skills` (workspace = repo root). Works with token router + `fork.opencode.env`.
