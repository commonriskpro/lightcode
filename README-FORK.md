...
- **Run a self-contained CLI** (no `~/.config` / global XDG for app data): `./scripts/opencode-isolated.sh` — sets `OPENCODE_PORTABLE_ROOT` to `<repo>/.local-opencode` and, if present, `OPENCODE_BIN_PATH` to `packages/opencode/dist/opencode-*/bin/opencode` from `bun run build -- --single`. Override `OPENCODE_BIN_PATH` yourself if needed. Build first: `(cd packages/opencode && bun run build -- --single)`.
