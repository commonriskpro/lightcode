...
**Eval offline (dataset JSONL + métricas sin modelo de chat):** `packages/opencode/docs/router-eval.md` — `bun run router:eval` desde `packages/opencode`. **Perfiles recomendados (A–D), snippets JSON y tabla de flags:** `packages/opencode/docs/tool-router-profiles.md`. **Puerta de regresión estricta (subset revisado congelado):** `bun run router:eval:reviewed:gate` (perfil **safe** / `per_turn_subset`; exige **100%** pass en `router-eval-reviewed.jsonl`). **Comparar modos de exposición sin memorizar flags:** `bun run router:eval:reviewed:subset-reminder`, `router:eval:reviewed:session-accumulative`, `router:eval:reviewed:compare-exposure`. **Batería multi-turn:** `bun run router:eval:scenarios`. El dataset **expanded** es exploratorio; ver docs para `router:eval:expanded:breakdown` y el advisory opcional. **Coste estimado de definiciones de tools:** `bun run router:eval:tool-costs` y `--breakdown` en el harness. **Modos de exposición** (`exposure_mode`): `router-eval.md` sección *Tool exposure modes*.
