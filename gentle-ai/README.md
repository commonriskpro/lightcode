...
Integration with this fork: default profile uses `initial_tool_tier: full` and `experimental.tool_router` with `apply_after_first_assistant: false` so the offline router selects tools from the **first** user message; `inject_prompt` adds intent + tool ids to the system prompt. Optional alternate profile: `minimal` + `apply_after_first_assistant: true` for a cheaper first turn. `fork.opencode.env` aligns env flags. `sdd-orchestrator` keeps `task` → `sdd-*` only for delegation targets; `bash` / `edit` / `write` follow normal permissions so the offline router can attach them when the user’s intent matches (e.g. delete, shell). Use **tab** in the TUI to pick `sdd-orchestrator`, or set `"default_agent": "sdd-orchestrator"` in `.opencode/opencode.jsonc` to use SDD by default.
