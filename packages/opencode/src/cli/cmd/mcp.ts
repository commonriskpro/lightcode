    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in lightcode.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .lightcode/ subdirectory too)
  const candidates = [path.join(baseDir, "lightcode.json"), path.join(baseDir, "lightcode.jsonc")]

  if (!global) {
    candidates.push(
      path.join(baseDir, ".lightcode", "lightcode.json"),
      path.join(baseDir, ".lightcode", "lightcode.jsonc"),
    )
  }
  ...
  // Default to lightcode.json if none exist
  return candidates[0]
}
