...
export namespace SessionPrompt {
  ...
  export async function resolveTools(input: {
    ...
    cfg?: Config.Info
    /** Current user message: fallback expansion budget is tracked per logical user turn. */
    ...
    const routed = await ToolRouter.apply({
      ...
      stickyToolIds,
      fallback: {
        expansionsUsedThisTurn: input.lastUser?.toolRouterFallbackExpansionsUsed ?? 0,
        maxPerTurn: trFb?.max_expansions_per_turn ?? 1,
        expandTo: trFb?.expand_to ?? "full",
        sessionID: input.session.id,
        messageID: input.processor.message.id,
        turn: userTurn,
      },
    })
    if (routed.usedFallbackExpansion && input.lastUser) {
      await Session.updateMessage({
        ...input.lastUser,
        toolRouterFallbackExpansionsUsed: (input.lastUser.toolRouterFallbackExpansionsUsed ?? 0) + 1,
      })
    }
    if (trace) {
      log.info("resolveTools.router", {
        ...
        sessionID: input.session.id,
        used_fallback_expansion: routed.usedFallbackExpansion === true,
      })
    }
