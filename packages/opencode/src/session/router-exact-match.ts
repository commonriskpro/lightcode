...
export type ExactMatchFlags = {
  ...
  /** When `dynamic_ratio` is true: multiplier vs best score for non-composite prompts. Default 0.97 (benchmark-tuned). */
  dynamic_ratio_simple?: number
  /** When `dynamic_ratio` is true: multiplier for composite / multi-step prompts. Default 0.74 (benchmark-tuned). */
...
export function effectiveAutoRatio(
  ...
  return compositeIntent(userText, intentLabel)
    ? (exact.dynamic_ratio_composite ?? 0.74)
    : (exact.dynamic_ratio_simple ?? 0.97)
}
