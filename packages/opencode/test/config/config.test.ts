...
async function writeManagedSettings(settings: object, filename = "lightcode.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "lightcode.json") {
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "lightcode.json"))
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "lightcode.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .opencode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".lightcode")
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const opencodeDir = path.join(projectDir, ".lightcode")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await Filesystem.write(
        path.join(opencodeDir, "lightcode.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".lightcode")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "lightcode.json")
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          ...
          const opencodeDir = path.join(dir, ".lightcode", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
